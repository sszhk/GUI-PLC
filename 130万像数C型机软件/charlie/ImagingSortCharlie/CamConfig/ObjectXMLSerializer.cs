...
namespace ImagingSortCharlie.Xml
{
  /// <summary>
  ...
  {
    public static bool SaveBinXml(string xml, T value)
    {
      ...
      ObjectXMLSerializer<T>.Save(value, xml, SerializedFormat.Binary);
      return true;
    }
    ...
    public static T LoadBinXml(string xml)
    {
      FileInfo fi = new FileInfo(xml);
      if (!fi.Exists)
        return null;
      if (fi.Length == 0)
        return null;
      return ObjectXMLSerializer<T>.Load(xml, SerializedFormat.Binary);
    }
