...
bool PGObj::start(int FPS, bool async)
{
  ...
  FlyCaptureError error = FLYCAPTURE_OK;
  error = flycaptureStart(ctx, FLYCAPTURE_VIDEOMODE_640x480Y8, fr);
...
//     {
      ...
      if( callback )
        ...
        co.serial, 
        cap_info.pData, 
        (int)(tm.wMilliseconds - cap_info.timeStamp.ulMicroSeconds / 1000), 
        cap_info.iRows);
      Sleep(0);
...
#ifdef _DEBUG
  printf("thrd_grab #%d %d exiting...\n", obj->cam_index, obj->co.serial);
#endif
  return obj->co.serial;
}
