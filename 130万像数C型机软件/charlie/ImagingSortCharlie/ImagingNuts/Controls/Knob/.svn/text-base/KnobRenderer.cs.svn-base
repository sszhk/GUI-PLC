...
namespace LBSoft.IndustrialCtrls.Knobs
{
	...
	public class LBKnobRenderer : LBRendererBase
	{
		...
		public virtual bool DrawScale( Graphics Gr, RectangleF rc )
		{
			...
			Color cKnob = Color.White;//this.Knob.ScaleColor;
      Color cKnobDark = Color.Black;//LBColorManager.StepColor ( cKnob, 60 );
      GraphicsPath gp = new GraphicsPath();
      rc.Inflate(-7, -7);
      gp.AddArc(rc, 135, 270);
      int count = gp.PathPoints.Length;
      //gp.AddPie(rc.Left, rc.Top, rc.Width, rc.Height, 0, 360);
      //gp.CloseAllFigures();
      //using (PathGradientBrush br = new PathGradientBrush(gp))
      float perimeter = (float)(Math.PI * rc.Height * 270 / 360);
      PointF pt1 = new PointF(0, 0);
      PointF pt2 = new PointF(pt1.X + perimeter, 0);
      RectangleF rc1 = rc;
      rc1.Inflate(7, 7);
      using (LinearGradientBrush br = new LinearGradientBrush(rc1, cKnob, cKnobDark, 0f))
      {
        //br.WrapMode = WrapMode.Clamp;
//         Blend blend = new Blend();
//         blend.Positions = new float[] { 0, 0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1};
//         blend.Factors = new float[] { 0, 0.1f, 0.2f, 0.3f, 0.4f, 0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1 };
//         //br.CenterColor = cKnobDark;
//         //br.SurroundColors = new Color[] {cKnob, cKnobDark};
//         br.Blend = blend;
        Pen p = new Pen(br);
        p.Width = 14;
        //br.Blend
